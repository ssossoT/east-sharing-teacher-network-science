...
import { auth, db, firebaseConfigured, missingFirebaseEnv, workspaceId } from "@/lib/firebase";
...
export function AuthProvider({children}:{children:React.ReactNode}) {
  ...
  const login=async(name:string,password:string,register=false)=>{
    if(!auth) throw new Error(`Vercel에 Firebase 환경 변수가 없습니다: ${missingFirebaseEnv.join(", ")}`);
